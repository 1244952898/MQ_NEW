        ...
        public JsonResult UploadImage(int type)
        {
            if (HttpContext.Request.Files.Count > 0)
            {
                ...
                try
                {
                    ...
                    if (file == null)
                        return Json(new JsonBaseEntity { ErrorCode = "10000", ErrorMessage = "上传文件为空！" });
                    ...
                    var oldFileName = file.FileName.Split('.')[0];
                    ...
                    return Json(new JsonUploadFileEntity { ErrorCode = "10001", ErrorMessage = "上传成功", FileUrl = string.Format("{0}/TuPian/Images/{1}/Show/{2}", "", type, newFileName), FileName = newFileName, OldFileName = oldFileName });
                }
                catch (Exception ex)
                {
